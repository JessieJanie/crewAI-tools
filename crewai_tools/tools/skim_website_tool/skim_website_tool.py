...
Exposes :class:`SkimWebsiteTool`, a CrewAI ``BaseTool`` that fetches any URL and
...
class SkimWebsiteToolSchema(BaseModel):
    """Input schema for :class:`SkimWebsiteTool`."""
...
class SkimWebsiteTool(BaseTool):
    ...
    name: str = "Skim web reader tool"
    description: str = _TOOL_DESCRIPTION
    args_schema: Type[BaseModel] = SkimWebsiteToolSchema
    ...
    package_dependencies: List[str] = ["x402", "eth-account", "requests"]
    env_vars: List[EnvVar] = [
        EnvVar(
            name="SKIM_WALLET_PRIVATE_KEY",
            description="Hex private key for the Base wallet that pays for reads over x402",
            required=True,
        ),
    ]
    ...
    def _get_session(self) -> Any:
        ...
        try:
            ...
            raise ImportError(
                "SkimWebsiteTool needs the x402 client with EVM support. Install "
        ...
        if not key:
            raise ValueError(
                ...
                "by passing private_key=... to SkimWebsiteTool(). The key never leaves your "
